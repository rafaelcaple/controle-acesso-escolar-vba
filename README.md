...
A student access control system that uses QR code badges to register school and cafeteria entry/exit, **deployed in production at CIE Miécimo da Silva, Rio de Janeiro**, developed between June 2022 and August 2023 using VBA for Microsoft Excel.

## Background
The school had no reliable way to track cafeteria entries, which led to inconsistencies. The system was built to solve that problem: students point their badge at a scanner, which sends the ID to the system to automatically record the entry, blocking duplicate access with the same student ID.
...
## My Contributions
This was a team project with 4 members. I was responsible for all technical development, including:
- Entry logging via QR code badge: a physical scanner reads the badge and passes the student ID to the system, which records the entry automatically
...
- Improved security for school entry and exit tracking
